...
from ...algorithms import astar_find_path, get_sorted_directions
...
class AStarPathfindingStrategy(PathfindingStrategy):
    ...
    def find_path(self, start: Position, end: Position,
        ...
        is_valid, result_path, all_obstacles = PathfindingValidator.validate_pathfinding_params(
            start, end, obstacles, width, height, extra_obstacles, "A*路径规划"
        )
        if not is_valid:
            return []
        ...
        def on_no_path(iterations: int, seen_count: int):
            # 找不到路径时静默，不输出日志
            pass
        
        return astar_find_path(
            start=start,
            end=end,
