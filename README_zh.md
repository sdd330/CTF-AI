...
Capture the Flag 是一个经典的团队竞技游戏，两个队伍在矩形地图上竞争。

### 基本规则

- **两个队伍**：**L 队**（左侧）和 **R 队**（右侧）
- **地图布局**：矩形地图，左侧是 L 队的领地，右侧是 R 队的领地
- **地图元素**：地图中包含障碍物（walls）和墙壁，玩家无法通过

### 队伍资源

每个队伍拥有以下资源：
- **领地（Territory）**：地图的一半区域（L 队在左侧，R 队在右侧）
- **目标区域（Target Area）**：位于己方领地内，用于得分
- **监狱（Prison）**：位于己方领地内，用于关押被标记的敌方玩家
- **旗子（Flags）**：位于己方领地内，初始位置在目标区域附近

### 游戏目标

**主要目标**：收集对方队伍的旗子并带回己方目标区域得分。
...
3. **监狱机制（Prison）**
   - 被标记的玩家会被送到标记方的监狱
   - 玩家在监狱中会停留一段时间（默认 20000 游戏 tick）
   - 监狱中的玩家无法移动或执行其他动作

4. **救援机制（Rescue Teammate）**
   - 队友可以前往**敌方监狱**救援被关押的队友
   - 救援需要救援者与被救援者在同一位置
   - 救援成功后，被救援的玩家立即恢复自由状态

5. **得分机制（Score Flag）**
   - 玩家必须持有敌方旗子
   - 玩家必须在**己方目标区域**内
   - 得分后，旗子会重置到原始位置，玩家恢复自由状态
...
游戏支持手动键盘控制，方便调试和测试：
- **L队（左侧）**：使用 `WASD` 键控制（W=上，A=左，S=下，D=右）
- **R队（右侧）**：使用方向键控制（↑↓←→）
- **空格键**：启动/暂停游戏
- **优先级**：键盘输入优先于AI远程控制（便于手动调试）

注：AI训练模式下会禁用键盘输入，完全由AI控制。
