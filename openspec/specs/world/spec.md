...
## Purpose
Central state container managing all game state, player coordination, and the game loop.
## Requirements
### Requirement: State Container
The World SHALL serve as the single source of truth for all game state.

#### Scenario: State initialization
- **WHEN** a World is created
- **THEN** all players, flags, teams, and map data are initialized

#### Scenario: State access
- **WHEN** any component needs game state
- **THEN** the World provides consistent, up-to-date information
...
- **THEN** `world.my_players` and `world.enemy_players` dictionaries are used
- **THEN** to get all players, combine dictionaries: `{**world.my_players, **world.enemy_players}`
...
### Requirement: Player Query Utilities
The World SHALL support querying players and flags through utility functions.

#### Scenario: List players
- **WHEN** `list_players(world)` is called
- **THEN** all players matching criteria are returned

#### Scenario: List flags
- **WHEN** `list_flags(world)` is called
- **THEN** all flags matching criteria are returned
